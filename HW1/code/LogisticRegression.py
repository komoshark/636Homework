...
class logistic_regression(object):
    ...
    def fit_BGD(self, X, y):
        ...
        for epoch in range(self.max_iter):
            _g = np.mean([self._gradient(x, y) for x,y in zip(X, y)], 0)
    ...
    def _gradient(self, _x, _y):
        ...
        """
		### YOUR CODE HERE
        _g = -(_x*_y)/(1 + np.exp(_y*np.dot(self.W, _x)))
        return _g
		### END YOUR CODE
    ...
    def predict_proba(self, X):
        ...
        """
		### YOUR CODE HERE
        preds = np.array((1 / (1 + np.exp(np.dot(self.W, x)))) for x in X)
        preds_proba = np.stack(preds, 1 - preds, axis = -1)
        return preds_proba
		### END YOUR CODE
