...
def prepare_X(raw_X):
	...
	### YOUR CODE HERE
    f_symmetry = -np.sum(np.abs(raw_image - np.flip(raw_image, axis=2)), axis=(1,2))/256.0
	### END YOUR CODE

	# Feature 2: Measure of Intensity
	### YOUR CODE HERE
    f_intensity = np.sum(raw_image, axis=(1,2))/256.0
	### END YOUR CODE
	...
	### YOUR CODE HERE
    X = np.stack([f_bias, f_symmetry, f_intensity], axis=1)
	### END YOUR CODE
    return X
